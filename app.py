...
def cities_by_country():
    if 'country_name' in request.args:
        ...
        country_id = [country_id.country_id for country_id in session.query(Country).filter(Country.country == country_name)][0]
        return json.dumps([city.city for city in session.query(City).filter(City.country_id == country_id).all()])
    if 'per_page' in request.args and 'page' in request.args:
        number_of_rows_per_page = int(request.args['per_page'])
        page_number = int(request.args['page'])
        return json.dumps([city.city for city in session.query(City).limit(number_of_rows_per_page).offset(page_number*number_of_rows_per_page).all()])
